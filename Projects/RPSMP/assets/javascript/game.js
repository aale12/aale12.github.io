...
function updatePlayerDisplay(player, playerData) {
    $("#name-" + player).html(playerData)
}
fP.on("value", function (s) {
    var fP1Choice = s.child("p1/choice").val();
    var fP2Choice = s.child("p2/choice").val();
    if (s.child("p1").exists() === true) {
        updatePlayerDisplay("player1", s.child("p1/name").val());
    ...
    if (s.child("p2").exists() === true) {
        updatePlayerDisplay("player2", s.child("p2/name").val());
...
function nextRound() {
    setTimeout(function () {
        ...
        gameText.text("Choose rock, paper, or scissors.")
        fP.once("value", function (s) {
            console.log(s.val());
            s.child("p1/choice/").val().update("");
            s.child("p2/choice/").val().update("");
        });
    }, 3000);
}
